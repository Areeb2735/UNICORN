...
img_emb, pt_emb, text_emb, relapse, RFS, _ = next(iter(train_loader))
img_emb = img_emb.to(device)
pt_emb = pt_emb.to(device)
...
RFS = RFS.to(device)
summary(model, input_data=[text_tokens, img_emb, pt_emb], depth=8, col_names=["input_size", "output_size", "num_params"],)
...
for i in pbar:
    model.train()
    for j, (img_emb, pt_emb, text_emb, relapse, RFS, _) in enumerate(train_loader):
        img_emb = img_emb.to(device)
        pt_emb = pt_emb.to(device)
        text_emb=tokenizer(text_emb, return_tensors="pt", padding="max_length", truncation=True, max_length=512).to(device)
        y = encode_survival(RFS, relapse, time_bins).to(device)
    ...
    if i > -1:
        if ci > best_ci:
            best_ci = ci
            best_epoch = i+1
            save_path = os.path.join("/opt/sagemaker/new_home/Mohammad.Qazi@mbzuai.ac.ae/project/ct_rate/save/text_img_adapter", 'weight_{}.pth'.format(str(best_epoch).zfill(3)))
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            torch.save(model.state_dict(), save_path)
            print(f"Model saved at epoch {best_epoch}")

# save this also in a txt file
best_ci = round(best_ci, 4)
best_epoch = best_epoch
os.makedirs(os.path.dirname('/opt/sagemaker/new_home/Mohammad.Qazi@mbzuai.ac.ae/project/ct_rate/save/text_img_adapter/result.txt'), exist_ok=True)
with open('/opt/sagemaker/new_home/Mohammad.Qazi@mbzuai.ac.ae/project/ct_rate/save/text_img_adapter/result.txt', 'w') as f:
