...
num_epochs = 100
verbose=True
optimizer = make_optimizer(Adam, model, lr=0.01, weight_decay=0)
pbar =  trange(num_epochs, disable=not verbose)
for i in pbar:
    for j, (video, text, relapse, RFS, _) in enumerate(train_loader):
        ...
        loss = mtlr_neg_log_likelihood(y_pred, y, model, C1=1, average=True)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
RFS_all = []
with torch.no_grad():
    for video, text, relapse, RFS in test_loader:
        video = video.to('cuda')
